 ...
 *
 *  1. Raw body. Signature verification must hash the exact bytes Cashfree
 ...
 *
 *     This matters more for Cashfree than it did for the previous provider:
 *     Cashfree signs the body with its decimal amounts formatted as sent
 *     (`170.00`), and a parse/re-serialise round trip turns that into `170`,
 *     which never matches.
 *
 *  2. No customer rate limiter. /api is wrapped in apiLimiter; Cashfree's
 ...
 * Cashfree, not a QBusto user, and the signature is the authentication.
 */

const express = require('express');

const logger = require('../config/logger');
const { handleCashfreeWebhook } = require('../controllers/paymentwebhook.controller');
 ...
 * 500 tells Cashfree the failure is transient and worth retrying, when a body
 ...
 * 5xx so Cashfree retries them — is passed straight through to the shared
 * handler untouched.
 *
 * These are answered with 4xx rather than 200 deliberately. A 2xx would tell
 * Cashfree the event was handled; nothing here was handled, and if such a
...
router.use((err, req, res, next) => {
  ...
  logger.warn('Cashfree webhook rejected before verification', {
    requestId: req.id,
    type: err.type,
