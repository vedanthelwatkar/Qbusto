...
const { seedOrders } = require('./seed-dev-orders');
...
async function report() {
  ...
  const totals = {
    ...
    Orders: cinemaIds.length ? await models.Order.count({ where: { cinemaId: cinemaIds } }) : 0,
  };
...
/**
 * Create a development admin user for dashboard access.
 *
 * Uses the first PVR chain as the owning chain for scope.
 */
async function createDevUser() {
  try {
    const pvrChainId = [...ids.chains.values()][0]; // First chain (PVR)

    // Check if user already exists
    const existing = await models.User.findOne({
      where: { username: 'qbusto-admin' },
      attributes: ['id'],
    });

    if (existing) {
      console.log(`  ${OK} user qbusto-admin already exists`);
      return;
    }

    // Create owner user (has access to all chains/cinemas)
    const user = await models.User.create({
      chainId: pvrChainId,
      cinemaId: null, // Null for owner role (not cinema-scoped)
      role: ROLES.OWNER,
      username: 'qbusto-admin',
      password: 'qbusto-password',
      firstName: 'QBusto',
      lastName: 'Admin',
      isActive: true,
    });
    ...
    console.log(`  ${OK} user: qbusto-admin (password: qbusto-password)`);
  } catch (error) {
    fail(`Failed to create development user: ${error.message}`);
  }
}
...
async function main() {
  ...
  const ordersCreated = await seedOrders(ids.cinemas);
  console.log(`  ${OK} orders: ${ordersCreated}`);
