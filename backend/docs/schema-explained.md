...
stores the event identifier in `event_id` with a UNIQUE constraint, so a
redelivery fails the insert rather than being caught by an application-side
...
ever describes an order this system created.

`outcome` (`received`, `applied`, `ignored`) and `reason` capture what the
handler decided and why, for later investigation.

The table records deliveries; it does not decide payment state. Moving an order
