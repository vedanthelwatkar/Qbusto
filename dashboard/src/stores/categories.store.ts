/**
 * The category list: query, results and the state of the request that produced
 * them.
 *
 * Mirrors users.store deliberately. Only the list lives here; create, edit and
...
const DEFAULT_QUERY: GetApiCategoriesParams = {
  page: 1,
  limit: 20,
  sort: 'createdAt',
  order: 'desc',
...
interface CategoriesState {
   ...
   */
  setQuery: (patch: Partial<GetApiCategoriesParams>) => void;
  fetch: () => Promise<void>;
  reset: () => void;
}
...
export const useCategoriesStore = create<CategoriesState>((set, get) => ({
  ...
  fetch: async () => {
    ...
    try {
      const { categories, pagination } = await categoriesService.listCategories(get().query);

      if (requestId !== latestRequest) return;

      set({ categories, pagination, loading: false });
    } catch (caught) {
      if (requestId !== latestRequest) return;
  ...
  reset: () => {
    latestRequest += 1;
    set({ query: DEFAULT_QUERY, categories: [], pagination: null, loading: false, error: null });
  },
}));
