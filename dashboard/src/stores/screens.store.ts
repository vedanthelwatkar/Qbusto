/**
 * The screen list: query, results and the state of the request that produced
 * them.
 *
 * Mirrors cinemas.store deliberately. Only the list lives here; create, edit
...
const DEFAULT_QUERY: GetApiScreensParams = {
  page: 1,
  limit: 20,
  sort: 'createdAt',
  order: 'desc',
...
interface ScreensState {
   ...
   */
  setQuery: (patch: Partial<GetApiScreensParams>) => void;
  fetch: () => Promise<void>;
  reset: () => void;
}
...
export const useScreensStore = create<ScreensState>((set, get) => ({
  ...
  fetch: async () => {
    ...
    try {
      const { screens, pagination } = await screensService.listScreens(get().query);

      if (requestId !== latestRequest) return;

      set({ screens, pagination, loading: false });
    } catch (caught) {
      if (requestId !== latestRequest) return;
  ...
  reset: () => {
    latestRequest += 1;
    set({ query: DEFAULT_QUERY, screens: [], pagination: null, loading: false, error: null });
  },
}));
