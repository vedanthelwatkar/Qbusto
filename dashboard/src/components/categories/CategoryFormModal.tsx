...
import { listChains } from '@/services/users.service';
...
export default function CategoryFormModal({ category, onClose, onSaved }: CategoryFormModalProps) {
  ...
  // Chains are authorised as Settings rather than Categories, so an editor
  // without that module gets a 403. The form falls back to a plain id field,
  // which the API accepts just the same.
  useEffect(() => {
    if (actor?.role !== 'owner' || isEdit) return;
