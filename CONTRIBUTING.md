...
The platform is on-premise. The file itself lives on the server filesystem and
SQL Server holds only the reference. Do not use cloud object storage, and do
not put image bytes in a `VARBINARY` column or a base64 string.

Four entities carry an image, and each has exactly one column for it:

| Entity   | Table        | Column           |
| -------- | ------------ | ---------------- |
| Banner   | `banners`    | `image_url`      |
| Category | `categories` | `image_url`      |
| Product  | `products`   | `image_url`      |
| Chain    | `chains`     | `logo_image_url` |

That single column holds one of two things, and every consumer reads it the
same way:

- an external address, stored exactly as entered - `https://example.com/a.jpg`
- an application path to a file on this server - `/uploads/products/9f2c….webp`

Do not add a second column to distinguish the two. The prefix already does,
and splitting them would force every reader to decide which one wins.

The database never holds a filesystem path. `/uploads/products/9f2c….webp` is
a URL path served by the backend, not a location on disk, so the storage root
can differ between servers - or move - without touching a single row.

The storage root is `FILE_STORAGE_PATH`, and there is exactly one for the whole
platform. A relative value resolves against the repository root - the same
anchor `scripts/generate-openapi.js` uses - so the default puts uploads in the
shared directory that already holds the generated specification:
...
Production sets an absolute path outside the source tree so a redeploy cannot
delete customer images.

The backend is the only application that touches the filesystem. Do not give
the Dashboard, Consumer or Kitchen an upload directory: the Dashboard uploads
through `POST /api/uploads/{entity}` and the others read through
`GET /uploads/...`.

Filenames are generated by the server: 16 random bytes plus the extension the
file's own signature proves. Never build a stored name from an uploaded one,
and never trust a declared content type - both are attacker-controlled.

Giving a new entity an image means adding it to `UPLOAD_ENTITIES` in
`src/services/upload.service.js`, which maps the entity to the permission
module that owns it. The route, the folder and the permission check all follow
from that one entry.
