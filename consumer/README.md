...
| Purpose | Method | Endpoint | Auth | Request | Response |
|---------|--------|----------|------|---------|----------|
| Create order + Razorpay order in single call | POST | `/api/consumer/orders` | **None** | Order details + items | Order with razorpayOrderId, amount, keyId |

**Backend source**: New consumer endpoint; separate from staff endpoint `/api/orders`

**This endpoint does ALL of these in one transaction:**
1. Validates cinema/context/screen/products
2. Calculates all prices server-side (paise arithmetic)
3. Creates internal order rows
4. Creates Razorpay order
5. Returns razorpayOrderId + razorpayKeyId + amount for frontend checkout
...
- `amount` is in paise (INR × 100); frontend MUST NOT recalculate
- `razorpayKeyId` is public Razorpay KEY_ID; safe to send to frontend
- `razorpayOrderId` was created by backend; passed to Razorpay checkout
...
- 500: Razorpay API error or database error

#### 10.8 Payment Signature Verification (NEW - CRITICAL)

| Purpose | Method | Endpoint | Auth | Request | Response |
|---------|--------|----------|------|---------|----------|
| Verify Razorpay payment signature + update order state | POST | `/api/consumer/orders/{orderId}/payment-verify` | **None** | Razorpay payment callback response | Updated order (paymentStatus: paid) |

**Backend source**: New consumer endpoint

**The ONLY endpoint that updates payment state. Frontend callback alone does NOT change order state.**
...
3. Construct verification string: `{razorpayOrderId}|{razorpayPaymentId}`
4. Compute HMAC-SHA256(verification string, RAZORPAY_KEY_SECRET)
5. Compare computed signature to provided signature
6. If signatures do NOT match → return 403 (tampered/invalid payment)
7. If signatures match → update order (paymentStatus: paid), store paymentId + signature
...
- 404: Order not found
- 409: Order payment status is not "pending" (already paid or failed)
- 500: Database error

**Security Note**: Razorpay callback alone NEVER changes order state. Backend verification is mandatory.
...
| POST /api/consumer/orders | POST | Create order + Razorpay order | **Critical** |
| POST /api/consumer/orders/{orderId}/payment-verify | POST | Verify Razorpay signature + update state | **Critical** |
...
POST /api/consumer/orders
  (cart items + customer info + cinemaId)
    ↓
Server creates order, returns orderId + razorpayOrderId
    ↓
Frontend stores orderId, razorpayOrderId locally
    ↓
Display order summary (read-only)
    ↓
User clicks "Confirm and Pay"
    ↓
Initiate Razorpay checkout (see Payment Flow section)
```
...
### Single, Definitive Flow

This is the ONLY documented payment flow. There is no alternate design.

#### Step 1: Create Order (Frontend Action)

Frontend submits checkout form:
...
#### Step 2: Backend Creates Order + Razorpay Order

Backend **MUST** do ALL of this server-side:

1. Validate cinema/context/screen/products exist and belong to same chain
2. Validate products are active and available (check date/time windows)
3. Validate cinema and product links exist
4. Fetch current ProductPricing for each product (cinema/product/dayOfWeek)
5. Apply source-specific discount (qr, seat_qr, kiosk, counter)
6. Calculate subtotal, discount, final total in paise (integer math)
7. Create internal `orders` row (status: initiated, paymentStatus: pending)
8. Create `order_items` rows (frozen snapshots: productName, unitPrice, quantity, lineTotal)
9. Create Razorpay order via Razorpay API
10. Store razorpayOrderId on order row
...
**Critical Notes:**
- `amount` is in paise (multiply by 100 from rupees)
- Frontend must NEVER recalculate or trust its cart total
- `razorpayKeyId` is the public KEY_ID; Razorpay secret never reaches frontend
- If Razorpay order creation fails, return 500 error; consumer must retry checkout

#### Step 4: Frontend Opens Razorpay Checkout

The frontend has orderId + razorpayOrderId + razorpayKeyId + amount from Step 3.

Frontend does NOT fetch a separate "payment init" endpoint.

```javascript
// PaymentPage.tsx or similar
import Razorpay from 'razorpay/dist/razorpay';

async function handlePaymentClick() {
  // order is the response from Step 3
  
  const options = {
    key: order.razorpayKeyId,           // Public KEY_ID from response
    amount: order.amount,                 // In paise (already calculated server-side)
    currency: order.currency,             // INR
    order_id: order.razorpayOrderId,      // Server-created Razorpay order
    name: cinemaName,
    description: `Order #${order.orderId}`,
    customer_id: order.customerMobile || '',
    email: order.customerEmail || '',
    contact: order.customerMobile || '',
    
    handler: async (response) => {
      // Razorpay indicates payment was processed
      // But we do NOT trust this callback alone
      // We must verify the signature server-side
...
#### Step 5: Frontend Calls Signature Verification

After Razorpay callback (success OR user closes):
...
2. Verify the order exists and paymentStatus is still "pending"
3. Verify razorpayOrderId matches the stored value
4. Construct verification string: `{razorpayOrderId}|{razorpayPaymentId}`
5. Compute HMAC-SHA256(verification string, RAZORPAY_SECRET)
6. Compare computed signature to provided razorpaySignature
7. If signatures DO NOT match → return 403 with error "Payment verification failed"
8. If signatures match → update order: paymentStatus = "paid", store razorpayPaymentId and razorpaySignature
9. Return success response

#### Step 7: Frontend Shows Confirmation

Only show success screen if backend verification succeeds (HTTP 200 from Step 6).

If verification fails or times out → show error "Payment could not be verified. Contact support with order #XYZ."

### No Additional Payment Endpoints

There is:
- ❌ NO `/api/consumer/orders/{id}/payment-init` endpoint
- ❌ NO `/api/consumer/orders/{id}/payment` endpoint
- ✅ ONLY POST `/api/consumer/orders/{id}/payment-verify` (and only to verify signature)
